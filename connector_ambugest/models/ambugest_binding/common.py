...
class AmbugestBinding(models.AbstractModel):
    ...
    All the models used as bindings between Ambugest and Odoo
    ...
    external_id = fields.Char(string='ID on Ambugest')

    _sql_constraints = [
        ('ambugest_uniq', 'unique(backend_id, external_id)',
         'A binding already exists with the same Ambugest ID.'),
    ]
    ...
    @job(default_channel='root.ambugest')
    @related_action(action='related_action_ambugest_link')
    @api.model
    def import_record(self, backend, external_id, force=False):
        """ Import an Ambugest record """
        with backend.work_on(self._name) as work:
